...
> **Thư mục:** `docs/mobile-integration/` — chỉ còn 4 files canonical (đã xoá 5 files lịch sử trao đổi ngày 2026-05-10)

- **Driver App:** `docs/mobile-integration/01-driver-app.md` — **Cập nhật 2026-05-10** (đồng bộ BE-FIX-1…5). Onboarding 3 bước, GPS push, breakdown, SignalR, FCM. Điểm cốt lõi: `finalUrl` là full URL (không phải path relative); `GET /vehicles/mine` thay thế 2-step query; enum serialize thành string; `/drivers/me` trả `404` khi chưa submit hồ sơ; login response có `userId` + `role`.
- **Customer App:** `docs/mobile-integration/02-customer-app.md` — **Rewritten 2026-05-09.** Đăng ký/đăng nhập, pricing estimate (AllowAnonymous), tạo đơn, danh sách đơn (cần truyền customerId), real-time tracking SignalR, COD + VNPay, escrow confirm/dispute, FCM.
- **Admin Portal:** `docs/mobile-integration/03-admin-portal.md` — **Rewritten 2026-05-09.** Auth (Admin tạo bởi Admin khác qua `/api/v1/admin/accounts`), driver verification queue, vehicle mgmt, shipment confirm/decline-dispatch, payment list + escrow resolve, analytics KPI/breakdown/fraud alert + acknowledge.
- **Driver App Gotchas:** `docs/mobile-integration/driver-app-gotchas.md` — **Cập nhật 2026-05-10** (thêm #11–13). Platform bugs đã fix (macOS Keychain, camera, logout loop, Firebase init order) + API gotchas (enum string, finalUrl full URL, 404 normal, /vehicles/mine).

**Files đã xoá (lịch sử trao đổi — không cần giữ):**
- `driver-registration-flow.md` — superseded bởi `01-driver-app.md`
- `driver-app-integration-review.md` — Q&A đã được trả lời và hành động
- `driver-app-backend-response.md` — đã thực hiện xong
- `driver-app-action-plan.md` — đã hoàn thành (BE-FIX-1→5)
- `driver-app-be-delivery-2026-05-10.md` — nội dung đã được merge vào `01-driver-app.md` + `gotchas.md`
