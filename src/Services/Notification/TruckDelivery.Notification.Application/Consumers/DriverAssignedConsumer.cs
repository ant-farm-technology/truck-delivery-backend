    ...
    : BackgroundService
{
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        // Notify driver via Push + SMS (Critical: driver assignment is time-sensitive)
        await mediator.Send(new SendNotificationCommand(
            RecipientId: @event.DriverId,
            Type: NotificationType.DriverAssigned,
            Channels: [NotificationChannel.Push, NotificationChannel.Sms],
            Title: "New delivery assigned",
            Body: $"You have been assigned to shipment {@event.ShipmentId}. Please confirm."), ct);

        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Notification sent for DriverAssigned ShipmentId={ShipmentId}", @event.ShipmentId);
    }
