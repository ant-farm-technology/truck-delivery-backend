...
public sealed class ShipmentStatusUpdatedConsumer(
    ...
    : BackgroundService
{
    ...
    private const string Topic = "shipment.shipment.status-updated";
    private const string DlqTopic = "shipment.shipment.status-updated.dlq";
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        var @event = JsonSerializer.Deserialize<ShipmentStatusUpdatedEvent>(result.Message.Value);
        if (@event is null)
        {
        ...
        var (type, channels, title, body) = @event.Status switch
        {
            "InProgress" => (
                NotificationType.ShipmentPickedUp,
                (IReadOnlyList<NotificationChannel>)[NotificationChannel.Push],
                "Shipment picked up",
                $"Your shipment {@event.ShipmentId} has been picked up and is on the way."),
            "Completed" => (
                NotificationType.ShipmentDelivered,
                (IReadOnlyList<NotificationChannel>)[NotificationChannel.Push, NotificationChannel.Email],
                "Shipment delivered",
                $"Your shipment {@event.ShipmentId} has been successfully delivered."),
            _ => default((NotificationType, IReadOnlyList<NotificationChannel>, string, string))
        };
        ...
        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Notification sent for ShipmentStatus={Status} ShipmentId={ShipmentId}",
            @event.Status, @event.ShipmentId);
    }
