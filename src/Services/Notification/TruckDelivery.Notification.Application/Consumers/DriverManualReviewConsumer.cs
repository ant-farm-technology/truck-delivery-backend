...
public sealed class DriverManualReviewConsumer(
    ConsumerConfig consumerConfig,
    IProducer<string, string> producer,
    IServiceScopeFactory scopeFactory,
    IConfiguration configuration,
    ILogger<DriverManualReviewConsumer> logger)
    : BackgroundService
{
    private static readonly ActivitySource ActivitySource = new("TruckDelivery.Kafka.Consumer");
    private static readonly TextMapPropagator Propagator = Propagators.DefaultTextMapPropagator;
    private const string Topic = "driver.driver.manual-review-required";
    private const string DlqTopic = "driver.driver.manual-review-required.dlq";

    private readonly IConsumer<string, string> _consumer =
        new ConsumerBuilder<string, string>(consumerConfig).Build();

    // Admin email read from config; if not set, email channel is skipped
    private readonly string? _adminEmail = configuration["Notification:AdminEmail"];
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        var @event = JsonSerializer.Deserialize<DriverManualReviewRequiredEvent>(result.Message.Value);
        if (@event is null)
        {
        ...
        await mediator.Send(new SendNotificationCommand(
            ...
            Type: NotificationType.DriverManualReviewRequired,
            Channels: channels,
            Title: "Driver requires manual verification",
            Body: $"Driver {@event.DriverName} (confidence: {@event.ConfidenceScore:P0}) is awaiting manual review.{(string.IsNullOrWhiteSpace(@event.Notes) ? "" : $" Notes: {@event.Notes}")}",
            RecipientEmail: _adminEmail), ct);

        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Admin notification sent for ManualReview DriverId={DriverId}", @event.DriverId);
    }
