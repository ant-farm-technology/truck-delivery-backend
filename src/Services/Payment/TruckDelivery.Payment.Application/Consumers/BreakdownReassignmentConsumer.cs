...
// Listens for breakdown reassignment events and locks a surcharge fee in escrow.
// The escrowed amount is released on delivery confirmation or refunded on dispute.
    ...
    ILogger<BreakdownReassignmentConsumer> logger) : BackgroundService
{
    ...
    // Fixed surcharge locked per breakdown event. In production, derive from order fee data.
    private const decimal BreakdownSurchargeFee = 50_000m; // 50,000 VND
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        var commandResult = await mediator.Send(new CreateEscrowCommand(
            @event.ShipmentId,
            @event.OrderId,
            @event.OriginalDriverId,
            @event.ReplacementDriverId,
            BreakdownSurchargeFee), ct);

        if (commandResult.IsFailure)
        {
            logger.LogWarning("CreateEscrow failed: {Error}", commandResult.Error.Description);
        ...
        if (logger.IsEnabled(LogLevel.Information))
            logger.LogInformation("Escrow locked for Shipment={ShipmentId} EscrowId={EscrowId}",
                @event.ShipmentId, commandResult.Value);
    }
