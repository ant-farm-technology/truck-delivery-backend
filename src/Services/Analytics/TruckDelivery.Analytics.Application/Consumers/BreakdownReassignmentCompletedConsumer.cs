...
using TruckDelivery.Analytics.Application.Commands.RecordReassignmentCompleted;
...
public sealed class BreakdownReassignmentCompletedConsumer(
    ConsumerConfig consumerConfig,
    IProducer<string, string> producer,
    IServiceScopeFactory scopeFactory,
    ILogger<BreakdownReassignmentCompletedConsumer> logger)
    : BackgroundService
{
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        await mediator.Send(new RecordReassignmentCompletedCommand(
            @event.ShipmentId, @event.OriginalDriverId, @event.ReplacementDriverId), ct);

        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Analytics: resolved breakdown for Shipment={ShipmentId}", @event.ShipmentId);
    }
