...
using TruckDelivery.Analytics.Application.Commands.RecordBreakdown;
...
public sealed class VehicleBreakdownConsumer(
    ...
    ILogger<VehicleBreakdownConsumer> logger)
    : BackgroundService
{
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        await mediator.Send(new RecordBreakdownCommand(
            @event.DriverId, @event.VehicleId,
            @event.FraudRiskLevel, @event.Latitude, @event.Longitude), ct);

        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Analytics: recorded breakdown for Driver={DriverId}", @event.DriverId);
    }
