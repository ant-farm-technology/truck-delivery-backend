...
using TruckDelivery.Analytics.Application.Commands.RecordFraudAlert;
using TruckDelivery.Analytics.Application.IntegrationEvents;
using TruckDelivery.Shared.Infrastructure.Messaging.Kafka.Idempotency;

namespace TruckDelivery.Analytics.Application.Consumers;

public sealed class SuspiciousDriverPairConsumer(
    ConsumerConfig consumerConfig,
    IProducer<string, string> producer,
    IServiceScopeFactory scopeFactory,
    ILogger<SuspiciousDriverPairConsumer> logger)
    : BackgroundService
{
    private static readonly ActivitySource ActivitySource = new("TruckDelivery.Kafka.Consumer");
    private static readonly TextMapPropagator Propagator = Propagators.DefaultTextMapPropagator;
    private const string Topic = "driver.fraud.suspicious-pair-detected";
    private const string DlqTopic = "driver.fraud.suspicious-pair-detected.dlq";
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        var @event = JsonSerializer.Deserialize<SuspiciousDriverPairDetectedEvent>(result.Message.Value);
        if (@event is null)
        {
        ...
        await mediator.Send(new RecordFraudAlertCommand(
            @event.OriginalDriverId, @event.ReplacementDriverId,
            @event.SwapCount, @event.DetectedAt), ct);

        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogWarning("Analytics: fraud alert recorded Pair={OriginalDriverId}↔{ReplacementDriverId}",
            @event.OriginalDriverId, @event.ReplacementDriverId);
    }
    ...
    private async Task RouteToDlqAsync(ConsumeResult<string, string> result, Exception ex, CancellationToken ct)
    {
        ...
        {
            logger.LogCritical(dlqEx, "Failed to route to DLQ — message lost!");
        }
    }
