...
// Consumes ShipmentCompletedEvent — transitions Order to Delivered so Payment service triggers
public sealed class ShipmentCompletedConsumer(
    ConsumerConfig consumerConfig,
    IProducer<string, string> producer,
    IServiceScopeFactory scopeFactory,
    ILogger<ShipmentCompletedConsumer> logger) : BackgroundService
{
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        var @event = JsonSerializer.Deserialize<ShipmentCompletedEvent>(result.Message.Value);
        if (@event is null)
        {
        ...
        var statusResult = await mediator.Send(
            new UpdateOrderStatusCommand(@event.OrderId, OrderStatus.Delivered), ct);

        if (statusResult.IsFailure)
        {
            logger.LogWarning("UpdateOrderStatus(Delivered) failed for OrderId={OrderId}: {Error}",
        ...
        logger.LogInformation("Order {OrderId} status updated to Delivered", @event.OrderId);
    }
