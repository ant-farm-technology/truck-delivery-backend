...
using TruckDelivery.Tracking.Application.Commands.StopTracking;
    ...
    : BackgroundService
{
    ...
    private async Task ProcessAsync(ConsumeResult<string, string> result, CancellationToken ct)
    {
        ...
        await mediator.Send(new StopTrackingCommand(@event.ShipmentId), ct);
        await idempotencyStore.MarkProcessedAsync(@event.MessageId, ct);
        logger.LogInformation("Tracking stopped for ShipmentId={ShipmentId}", @event.ShipmentId);
    }
